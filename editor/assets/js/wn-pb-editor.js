...
jQuery(document).ready(function ($) {
    ...
    function get_the_trigger(this_widget, trigger) {
        ...
        let cube_properties_and_values =
            StylesConstructor.instance().get_styles_object(cube_id);
        ...
        if (cube_properties_and_values) {
            ...
            try {
                Object.keys(cube_properties_and_values).forEach(function (
                    display,
                    index
                ) {
                    Object.keys(this[display]).forEach(function (
                        property,
                        index
                    ) {
                        if (property == "display" && this[property] == "none") {
                            widget_wrapper_extra_classes +=
                                "wn_pb_e_widget_hidden ";
        ...
        if (
            $(this_widget).closest(".wn_pb_section_index").length &&
            !$(this_widget).closest(".wn_pb_index_resume").length &&
            !$(this_widget).closest(".wn_pb_index_resume_ext").length
        ) {
        ...
        if (!$(this_widget).parent().hasClass("wn_pb_e_widget_wrapper")) {
            $(this_widget).wrap(
                '<div class="wn_pb_e_widget_wrapper ' +
                    widget_wrapper_extra_classes +
                    '"></div>'
            );
            ...
            $(this_widget)
                .parent()
                .prepend('<div class="wn_pb_e_edit_btn"></div>');
    ...
    function register_all_editors() {
        ...
        $(".wn_pb_wrapper")
            ...
            .each(function () {
                ...
                if (
                    $(this).attr("cube-id") == undefined ||
                    $(this).attr("cube-id") == ""
                ) {
                    //Assigning a random id
                    $(this).attr(
                        "cube-id",
                        "wn_cb_" + get_radom_int(10, 50000)
                    );
                }
    ...
    function extra_cont_editor(element) {
        ...
        let editor_input_title = $(widget_editor).find(
            "#wn_pb_e_extra_cont_title"
        );
        let title_value = $(element).find(".wn_pb_link_text").text();
        $(editor_input_title).val(title_value);

        let editor_input_headline = $(widget_editor).find(
            "#wn_pb_e_extra_cont_headline"
        );
        let headline_value = $(element)
            .find(".wn_pb_extra_content > h3")
            .text();
        $(editor_input_headline).val(headline_value);

        let editor_input_textarea = $(widget_editor).find(
            "#wn_pb_e_extra_cont_textarea"
        );
        ...
        if (is_text_p) {
            ...
            textarea_value = $(element)
                .find(".wn_pb_extra_content .wn_pb_extra_content_text")
                .html();
        }

        $(editor_input_textarea).val(textarea_value);

        const img_url = $(element)
            .find(".wn_pb_extra_info_bck_img")
            .attr("img-url");
        ...
        start_listening(
            editor_input_title,
            $(element).find(".wn_pb_link_text")
        );

        start_listening(
            editor_input_headline,
            $(element).find(".wn_pb_extra_content > h3")
        );

        //Updating the modal
        start_listening(
            editor_input_headline,
            $("#myModal").find(".wn_pb_extra_content > h3")
        );

        if (is_text_p) {
            start_listening(
                editor_input_textarea,
                $(element).find(".wn_pb_extra_content > p"),
                "html"
            );
            start_listening(
                editor_input_textarea,
                $("#myModal").find(".wn_pb_extra_content > p"),
                "html"
            );
        } else {
            start_listening(
                editor_input_textarea,
                $(element).find(
                    ".wn_pb_extra_content .wn_pb_extra_content_text"
                ),
                "html"
            );
            start_listening(
        ...
        image_picker(
            media_lib_trigger,
            $(element).find(".wn_pb_extra_info_bck_img"),
            true
        );
    }
    ...
    function simple_video_editor(element) {
        ...
        let widget_editor = $(".wn_pb_e_widgets__editors")
            .find(".wn_pb_simple_video_editor")
            .clone();
        ...
        const video_src = $(video_element).attr("src"),
            video_iframe_src = $(element)
                .find(".wn_pb_video_element_iframe")
                .attr("src"),
        ...
        let media_lib_video_trigger =
            $(widget_editor).find(".wn_pb_e_sv_select");

        start_listening(
            $(widget_editor).find(".wn_pb_e_input_text"),
            text_element
        );
        ...
        $(widget_editor)
            .find(".wn_pb_e_video_url_input")
            .on("input", function () {
                let value = $(this).val();
                value = "https://www.youtube.com/embed/" + value;

                $(element)
                    .find('.wn_pb_video_element[value="1"]')
                    .attr("src", value);
            });

        $(element)
            .find(".wn_pb_video_element")
            .each(function () {
                if ($(this).css("display") == "block") {
                    const value = $(this).attr("value");
                    $(widget_editor)
                        .find(".wn_pb_e_select_index select")
                        .val(value);
                    let select = $(widget_editor).find(
                        ".wn_pb_e_select_index select"
                    );
    ...
    function register_source_selector(widget_editor, element) {
        $(widget_editor)
            .find(".wn_pb_e_select select")
            .each(function () {
                $(this).on("input", function () {
                    let selected_option = $(this).find(":selected");

                    let index_value = $(selected_option).attr("value");
                    let select_content_divs = $(selected_option)
                        .closest(".wn_pb_e_select")
                        .find(".wn_pb_e_select_content");

                    $(select_content_divs).removeClass(
                        "wn_pb_e_select_content__active"
                    );
                    $(select_content_divs)
                        .filter('[value="' + index_value + '"]')
                        .addClass("wn_pb_e_select_content__active");
    ...
    function simple_img_editor(element) {
        ...
        $(widget_editor)
            .find(".wn_pb_e_si_preview > img")
            .attr("src", element_src);
    ...
    function dropdown_editor(element) {
        ...
        $(element)
            .find(".wn_pb_e_dropdown_content option")
            .each(function () {
        ...
        $(widget_editor)
            .find(".wn_pb_e_input_dropdown_title")
            .on("change", function () {
        ...
        $(widget_editor)
            .find(".wn_pb_e_input_dropdown_label")
            .on("input", function () {
    ...
    function generic_editor(element) {
        let cube_id = $(element).attr("cube-id");
        let widgets_type_for_single_text = [
            "span",
            "h1",
            "h2",
            "h3",
        ...
        $(widget_editor)
            ...
            .each(function () {
                ...
                $.each(this_input_targets, function (index, value) {
                    ...
                    let collect_text_elements_type = [
                        "SPAN",
                        "H1",
                        "H2",
                        "H3",
    ...
    function register_responsivenes_checkboxes(widget_editor) {
        ...
        function check_for_any_active_checkbox() {
            let is_true = false;
            $(widget_editor)
                .find(".wn_pb_e_responsivenes_checkboxes input")
                .each(function () {
                    if ($(this).is(":checked")) {
                        is_true = true;
                    }
        ...
        $(widget_editor)
            ...
            .each(function () {
                ...
                if (cube_properties_and_values) {
                    if (
                        cube_properties_and_values[device_to_hide]["display"] ==
                        "none"
                    ) {
                ...
                $(this).on("input", function () {
                    ...
                    if ($(this).is(":checked")) {
                        $('[cube-id="' + cube_id + '"]')
                            .closest(".wn_pb_e_widget_wrapper")
                            .addClass("wn_pb_e_widget_hidden");

                        StylesConstructor.instance().add_style(
                            cube_id,
                            display_none,
                            device_to_hide
                        );
                        ...
                        if (!check_for_any_active_checkbox()) {
                            $('[cube-id="' + cube_id + '"]')
                                .closest(".wn_pb_e_widget_wrapper")
                                .removeClass("wn_pb_e_widget_hidden");
                        }

                        StylesConstructor.instance().remove_style(
                            cube_id,
                            display_none,
                            device_to_hide
                        );
        ...
        $(".wn_pb_f_editor_content")
            ...
            .each(function () {
                ...
                $(this).on("click", function () {
                    $(this)
                        .closest(".wn_pb_hle_tabs")
                        .find(".wn_pb_e_tab")
                        .removeClass("wn_pb_e_tab__active");

                    let editor = $(this).closest(".wn_pb_e_editors");
                    let current_content_height = $(editor)
                        .find(".wn_pb_e_content")
                        .height();

                    $(editor)
                        .find(".wn_pb_e_content")
                        .css("height", current_content_height + "px");
                    ...
                    if ($(this).hasClass("wn_pb_e_tab__content")) {
                        content_height = $(editor)
                            .find(".wn_pb_e_content__content")
                            .height();

                        $(this).addClass("wn_pb_e_tab__active");
                        $(editor)
                            .find(".wn_pb_e_content__content")
                            .addClass("wn_pb_e_content__active");
                    }
                    if ($(this).hasClass("wn_pb_e_tab__style")) {
                        content_height = $(editor)
                            .find(".wn_pb_e_content__style")
                            .height();
    ...
    function remove_element_event(
        ...
        if (widget_type == "button") {
            ...
            $(widget_editor)
                .find(".en_pb_e_remove_element")
                .on("click", function () {
                    let buttons_wrapper = $(element).closest(
                        ".wn_pb_selectors_wrapper"
                    );

                    $(element).remove();

                    if (
                        !$(buttons_wrapper).find(".wn_pb_selector_button")
                            .length
                    ) {
                ...
                .on("click", function () {
                    ...
                    if (is_index_element) {
                        ...
                        const index_number = $(element)
                            ...
                            .attr("index");
                        $(
                            '.wn_pb_wrapper > .wn_pb_container > section[index="' +
                                index_number +
                                '"]'
                        ).remove();
    ...
    function register_add_new_section_btn() {
        $(".wn_pb_e_add_new_section").on("click", function () {
            let new_section_template = $(
                ".wn_pb_e_new_section_template > section"
            ).clone();
    ...
    function update_index() {
        ...
        $(".wn_pb_container > section").each(function () {
            if (
                $(this).hasClass("wn_pb_resume_section") ||
                $(this).hasClass("wn_pb_resume_section--extension")
            ) {
                return;
            }
            ...
            let new_li =
                ...
                '" class="' +
                active_li +
                '"><a href="#"><i class="wn_pb_index_icon"></i><span class="wn_pb_e_headline_content" cube-id="" widget-id="" widget-type="span" >' +
                section_title +
                "</span></a></li>";
    ...
    function restart_the_frontend_script(coming_from) {
        ...
        switch (coming_from) {
            case "new_widget":
                move_to_section = WnPbFrontEnd.instance().active_section;
                break;
            case "new_section":
                move_to_section =
                    WnPbFrontEnd.instance().number_of_editable_sections;
                break;
            case "delete_element":
                move_to_section = WnPbFrontEnd.instance().active_section;
                break;
    ...
    function restart(coming_from) {
        ...
        //Removing empty widgets wrapppers
        $(".wn_pb_wrapper .wn_pb_e_widget_wrapper").each(function () {
            let widget = $(this).find("[cube-id]");
            if (!$(widget).length) {
                $(this).remove();
            }
        });
    ...
    function build_json_from_html(is_saving_data = false) {
        //Info
        product_data_2["info"] = {};
        product_data_2["info"]["product_name"] = $(
            "#wn_pb_produc_name_input"
        ).val();

        product_data_2["info"]["disable_snap_effect_on_mobile"] = $(
            "#wn_pb_disable_snap_effect__mobile"
        ).is(":checked")
            ? "true"
            : "";
        ...
        let logo_data = collect_from_widget_type(
            "simple-image",
            $(".wn_pb_logo img")
        );

        product_data_2["header"]["logo_widget"] = logo_data;
        product_data_2["header"]["reset_button_txt"] = $(
            '.wn_pb_header_content span[widget-id="h-reset_button_txt"]'
        ).text();

        //Footer
        product_data_2["footer"] = {};
        product_data_2["footer"]["est_cost_txt"] = $(
            ".wn_pb_total_cost_txt"
        ).text();
        product_data_2["footer"]["help_btn"] = {};
        product_data_2["footer"]["help_btn"]["help_btn_txt"] =
            $(".wn_pb_help_button").text();
        product_data_2["footer"]["help_btn"]["help_btn_url"] =
            "https://test.com";
        ...
        $(".wn_pb_container > section").each(function () {
            ...
            //Title
            product_data_2["sections"][this_section_id]["title"] =
                product_data_2["index"][section_cont];
    ...
    function collect_from_widget_type(widget_type, element) {
        ...
        if (widget_type == "form") {
            data = {};
            const cf7_shortcode = $(element)
                .find(".cf7_shortcode_content")
                .text();
            ...
            data["styles"] = StylesConstructor.instance().get_html_styles(
                data["cube_id"]
            );

            let styles_array = StylesConstructor.instance().get_styles_object(
                data["cube_id"]
            );
    ...
    function collect_html_data(element, type) {
        ...
        let all_styles_selectors_for_given_cube_id =
            StylesConstructor.instance().get_all_cube_styles_id(
                data["cube_id"]
            );
        data["styles"] = [];
        data["object_styles"] = {};
        ...
        if (all_styles_selectors_for_given_cube_id != null) {
            $.each(
                all_styles_selectors_for_given_cube_id,
                function (index, cube_id) {
                    ...
                    data["styles"].push(
                        StylesConstructor.instance().get_html_styles(
                            just_cube_id,
                            just_subtarget
                        )
                    );
    ...
    function video_picker(media_lib_trigger, element, widget_editor) {
        $(media_lib_trigger).on("click", function (e) {
            ...
            image_frame.on("select", function () {
                let datos_imagen = image_frame
                    .state()
                    .get("selection")
                    .first()
                    .toJSON();
                $(widget_editor)
                    .find(".wn_pb_e_sv_video_p")
                    .attr("src", datos_imagen["url"]);
    ...
    function image_picker(
        ...
        $(media_lib_trigger).on("click", function (e) {
            ...
            image_frame.on("select", function () {
                ...
                jQuery(".wn_pb_e_si_preview > img").attr(
                    "src",
                    datos_imagen["url"]
                );
                ...
                if (bck_img) {
                    $("#myModal")
                        ...
                        .attr(
                            "style",
                            "background-image: url(" + datos_imagen["url"] + ")"
                        );
                    $(element).attr(
                        "style",
                        "background-image: url(" + datos_imagen["url"] + ")"
    ...
    function register_add_new_wdiget_element() {
        $(".wn_pb_wrapper .wn_pb_options").each(function () {
            let current_add_button = $(this).find(".wn_pb_e_add_new_widget");

            if ($(current_add_button).length) {
                $(current_add_button).remove();
            }

            let add_widget_btn =
                '<div class="wn_pb_e_add_new_widget wn_pb_e_add_new_widget__options"><span>Add Widget</span></div>';
        ...
        $(".wn_pb_wrapper .wn_pb_media_wrapper").each(function () {
            ...
            let add_widget_btn =
                '<div class="wn_pb_e_add_new_widget wn_pb_e_add_new_widget__media"><span>Add Widget</span></div>';
            $(this).append(add_widget_btn);
        });
            ...
            $(widget_pickers)
                ...
                .each(function () {
                    $(this).on("click", function () {
                        ...
                        let widget_content = $(this)
                            .find(".wn_pb_e_widget_content")
                            .children();
    ...
    function append_button_widget(column, widget_content) {
        ...
        if (buttons_wrapper.length) {
            ...
            buttons_wrapper =
                '<div class="wn_pb_selectors_wrapper wn_pb_two_col_layout"></div>';
    ...
    function reorganize_sections(
        move_this_section_index,
        right_after_this_section_index
    ) {
        build_json_from_html(true);

        move_this_section_index = parseInt(move_this_section_index);
        right_after_this_section_index = parseInt(
            right_after_this_section_index
        );
        ...
        $(".wn_pb_section_index ul li").each(function (i) {
            if (
                $(this).hasClass("wn_pb_index_resume") ||
                $(this).hasClass("wn_pb_index_resume_ext")
            ) {
                return;
            }

            let headline_content = $(this)
                .find(".wn_pb_e_headline_content")
                .text();
    ...
    function register_draggable_index() {
        $(".wn_pb_section_index .wn_pb_e_index_wrapper").each(function () {
            ...
            $(this).on("mouseup", function (e) {
                ...
                let right_after_this_section_index = $(ul)
                    .find("li[move-below-this]")
                    .attr("index");
                ...
                if (!$(move_below_this).length) {
                    ...
                    if (prev_li_index < 0) {
                        ...
                        move_below_this = $(ul).find(
                            'li[index="' + prev_li_index + '"]'
                        );
                    }
                }
            ...
            $(document).on("mousemove", function (e) {
                if (click_down) {
                    ...
                    $(".wn_pb_section_index .wn_pb_e_index_wrapper").each(
                        function () {
                            ...
                            let top_margin = parseInt(
                                $(this_li_two).css("margin-top")
                            );
                            let evaluate_closeure =
                                relY - other_li_y_pos - top_margin - 20;
    ...
    function updating_to_version_1_0() {
        $(".wn_pb_wrapper .wn_pb_container")
            ...
            .each(function () {
                ...
                let new_div_element =
                    '<div class="wn_pb_extra_content_text">' +
                    current_content +
                    "</div>";
        ...
        $(".wn_pb_wrapper .wn_pb_container")
            .find(".wn_pb_video_content .wn_pb_modal_content_to_send")
            .each(function () {
    ...
    function updating_to_version_2_0() {
        let all_options_descriptions = $(".wn_pb_wrapper").find(
            ".wm_pb_options_description"
        );
    ...
    function removing_garbage_widgets() {
        //removing empty description widgets
        $(".wn_pb_editor .wm_pb_options_description").each(function () {
            let content = $(this).text();
            if (content == "" || content == " ") {
                if ($(this).parent().hasClass("wn_pb_e_widget_wrapper")) {
                    $(this).parent().remove();
                } else {
                    $(this).remove();
                }
...
class StylesConstructor {
            ...
            if (
                ...
                for (let column_index in section) {
                    if (
                        ...
                        for (
                            let column_i = 0;
                            column_i < column.length;
                            column_i++
                        ) {
                            let widget = column[column_i];

                            if (
                                widget["cube_id"] === undefined ||
                                widget["object_styles"] === undefined
                            ) {
                                continue;
                            }
                            ...
                            if (
                                typeof object_styles["desktop"] === "undefined"
                            ) {
                                jQuery.each(
                                    object_styles,
                                    function (subtarget, properties) {
                                        let array_styles = [];
                                        ...
                                        for (let property in properties[
                                            ...
                                            array_styles["desktop"][property] =
                                                properties["desktop"][property];
                                        }
                                        for (let property in properties[
                                            "tablet"
                                        ]) {
                                            array_styles["tablet"][property] =
                                                properties["tablet"][property];
                                        }
                                        for (let property in properties[
                                            "mobile"
                                        ]) {
                                            array_styles["mobile"][property] =
                                                properties["mobile"][property];
                                        }

                                        self.set_styles_object(
                                            subtarget,
                                            array_styles
                                        );
                                    }
                                ...
                                for (let property in object_styles["desktop"]) {
                                    array_styles["desktop"][property] =
                                        object_styles["desktop"][property];
                                }
                                for (let property in object_styles["tablet"]) {
                                    array_styles["tablet"][property] =
                                        object_styles["tablet"][property];
                                }
                                for (let property in object_styles["mobile"]) {
                                    array_styles["mobile"][property] =
                                        object_styles["mobile"][property];
                                }
    ...
    static get_html_styles(cube_id, subtarget = null) {
        ...
        for (let i = 0; i < 3; i++) {
            if (!jQuery(ansestor).hasClass("wn_pb_e_widget_wrapper")) {
                let ancestor_classes_array = jQuery(ansestor)
                    .attr("class")
                    .split(" ");

                if (ancestor_classes_array[0] !== undefined) {
                    cube_element_ansestors =
                        "." +
                        ancestor_classes_array[0] +
                        " " +
                        cube_element_ansestors;
                }
            }
    ...
    static render_styles(target) {
        ...
        let current_style = jQuery("head").find(
            '[cube-style-id="' + style_selector + '"]'
        );
        let html;
...
class Controls {
    ...
    set_property_targets() {
        ...
        jQuery(self.control)
            .find(".wn_cb_controls_inputs > input:not([mirror])")
            .each(function () {
    ...
    setting_inital_values() {
        let self = this;
        let cube_properties_and_values =
            StylesConstructor.instance().get_styles_object(self.cube_id);

        if (cube_properties_and_values != null) {
            self.properties_and_values = self.filter_taget_property(
                cube_properties_and_values
            );

            if (
                typeof self.properties_and_values["desktop"] === "undefined" &&
                typeof self.properties_and_values["tablet"] === "undefined" &&
                typeof self.properties_and_values["mobile"] === "undefined"
            ) {
    ...
    update_the_inputs_values() {
        ...
        jQuery(self.control)
            .find(".wn_cb_controls_inputs > input")
            .each(function () {
                if (values == undefined) {
                    jQuery(this).val("");
                } else {
                    ...
                    if (
                        typeof mirror !== typeof undefined &&
                        mirror !== false
                    ) {
    ...
    settings_values_to_zero() {
        ...
        jQuery(self.control)
            .find(".wn_cb_controls_inputs > [prop-target]")
            .each(function () {
        ...
        jQuery(self.control)
            .find(".wn_cb_controls_inputs > input")
            .each(function (this_input) {
                let property_name;
                let value;
                let attr_mirror = jQuery(this).attr("mirror");

                if (
                    typeof attr_mirror !== typeof undefined &&
                    attr_mirror !== false
                ) {
                    let mirror_target = jQuery(this).attr("mirror");
                    property_name = jQuery(self.control)
                        .find(mirror_target)
                        .attr("prop-target");
                } else {
                ...
                value = parseInt(
                    self.properties_and_values["desktop"][property_name]
                );
                jQuery(this).val(value);
            });
    }
    ...
    register_responsive_selector() {
        ...
        jQuery(self.control)
            .find(".wn_cb_controls_resp i")
            .on("click", function () {
                jQuery(self.control)
                    .find(".wn_cb_controls_resp i")
                    .removeClass("wn_cb_controls_resp_icon__active");
                jQuery(this).addClass("wn_cb_controls_resp_icon__active");

                self.current_resp_selector =
                    jQuery(this).attr("selector-value");
    ...
    register_unit_selector() {
        ...
        jQuery(self.control)
            .find(".wn_cb_controls_units_selector > span")
            .on("click", function () {
                jQuery(self.control)
                    .find(".wn_cb_controls_units_selector > span")
                    .removeClass("wn_cb_controls_units__active");
                jQuery(this).addClass("wn_cb_controls_units__active");
                self.current_unit_selector = jQuery(this).text();

                jQuery(self.control)
                    .find(".wn_cb_controls_dimensions_inputs > input")
                    .first()
                    .trigger("input");
            });
    }
...
class CheckboxControl extends Controls {
    ...
    register_checkbox_control(control) {
        ...
        let checkbox_swtich = jQuery(control).find(
            ".wn_pb_e_checkbox_switch > input"
        );
        let selected_status;
    ...
    setting_inital_value() {
        let self = this;
        let cube_properties_and_values =
            StylesConstructor.instance().get_styles_object(
                self.cube_id + self.target
            );

        if (cube_properties_and_values != null) {
            if (
                cube_properties_and_values["desktop"][self.property] ==
                this.status[0]
            ) {
                jQuery(self.control)
                    .find(".wn_pb_e_checkbox_switch > input")
                    .attr("checked", false);
            } else {
                jQuery(self.control)
                    .find(".wn_pb_e_checkbox_switch > input")
...
class SilderRangeControl extends Controls {
    constructor(control, cube_id) {
        ...
        this.current_resp_selector = jQuery(this.control)
            .find(".wn_cb_controls_resp > i")
            .first()
            .attr("selector-value");
        this.current_unit_selector = jQuery(this.control)
            .find(".wn_cb_controls_units_selector > span")
            .first()
            .text();
    ...
    watching_cahnges(input) {
        ...
        jQuery(input).on("input", function () {
            ...
            jQuery(self.control)
                ...
                .not("[mirror]")
                .each(function () {
                    let current_target_property_target =
                        jQuery(this).attr("prop-target");
                    properties[current_target_property_target] =
                        jQuery(this).val() + self.current_unit_selector;
                });
...
class DimensionsControl extends Controls {
    ...
    watching_cahnges(input, current_target) {
        ...
        jQuery(input).on("input", function () {
            ...
            let linked_selector = jQuery(self.control).find(
                ".wn_cb_controls_anchor_select"
            );
            let properties = [];
            ...
            jQuery(self.control)
                ...
                .each(function () {
                    ...
                    if (
                        jQuery(linked_selector).hasClass(
                            "wn_cb_controls_anchor__linked"
                        )
                    ) {
    ...
    register_anchor_selector() {
        ...
        let anchor_selector = jQuery(this.control).find(
            ".wn_cb_controls_anchor_select"
        );
